...
// Rescanner is the callback the watcher invokes for a mod folder that changed.
...
// rescan requests for the affected direct-child mod folder.
type Watcher struct {
	w     *fsnotify.Watcher
	rs    Rescanner
	mu    sync.Mutex
...
// handle determines which mod folder is affected by the event path
...
func (w *Watcher) handle(ev fsnotify.Event) {
	...
	for _, r := range roots {
		...
		root = r
		// First component of rel is the mod folder name.
		sep := string(filepath.Separator)
		idx := indexOfSep(rel, sep)
		if idx < 0 {
			modFolder = filepath.Join(r, rel)
		} else {
			modFolder = filepath.Join(r, rel[:idx])
		}
		break
	}
	if root == "" {
	...
	w.scheduleRescan(modFolder)
}

func (w *Watcher) scheduleRescan(modFolder string) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if t, ok := w.timers[modFolder]; ok {
		t.Stop()
	}
	w.timers[modFolder] = time.AfterFunc(w.debounce, func() {
		w.mu.Lock()
		delete(w.timers, modFolder)
		w.mu.Unlock()
		w.rs.RescanFolder(modFolder)
	})
}
